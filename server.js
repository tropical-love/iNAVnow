...
const DOM_INDEX = [
  [/코스피\s*200/, 'KPI200', '코스피 200'],
  [/코스닥\s*150/, null, '코스닥 150'], // 네이버에 실시간 지수코드 없음 → 참고치 생략
  [/코스피(?!\s*\d)/, 'KOSPI', '코스피'],
  [/코스닥(?!\s*\d)/, 'KOSDAQ', '코스닥'],
];
...
async function computeDomesticLev(stockCode, analysis, basic) {
  ...
  if (idxCode) {
    try {
      ...
      const cur = parseFloat(d.closePrice.replace(/,/g, ''));
  ...
  return {
    ...
    premiumPct: premiumOf(basic, mp, official, official, official).pct,
    premiumBasis: premiumOf(basic, mp, official, official, official).basis,
    inavRegular: official, sessContrib: 0, // 공식 iNAV는 정규장 기준 — 시장가와 시점 동일
    etfClose: etfCloseOf(basic), vsClosePct: etfCloseOf(basic) ? (official / etfCloseOf(basic) - 1) * 100 : null,
    inavChangePct: (official / navRef - 1) * 100,
    domContrib: (official / navRef - 1) * 100, frnContrib: 0, fxContrib: 0,
    ...
    note: `국내 ${lev > 0 ? '레버리지' : '인버스'} ${lev}배 — 선물 기반이고 실시간 선물 시세는 공개 소스가 없어, 자체 추정 대신 KRX 공식 iNAV를 그대로 표시합니다(본장 중 실시간 산출).`
      + (idx ? ` 참고로 ${idx.label} ${idx.value.toLocaleString('ko-KR')}(${idx.changePct >= 0 ? '+' : ''}${idx.changePct.toFixed(2)}%)를 ${lev}배 단순환산하면 ${spotEst.toLocaleString('ko-KR', { maximumFractionDigits: spotEst < 1000 ? 2 : 0 })}원 — 공식값과의 차이가 선물 베이시스입니다.` : '')
      + (idx && !idx.live ? ' 지수는 본장 마감으로 산출 종료.' : ''),
    rows: [], moreCount: 0, pdfDate: '-',
    asOf: new Date().toISOString(),
  };
}
...
async function computeINav(stockCode, depth = 0) {
  ...
  const rows = holdings.map(h => {
    ...
    const closed = q.session === '휴장' || q.session === '장마감'; // 진행 중 세션 없음
    const reg = q.regClose || q.prevClose;
    const sessPct = (!inReg && !closed && reg && q.last !== reg) ? (q.last / reg - 1) * 100 : null;
...
function render(){
  ...
  document.getElementById('stats').innerHTML =
    ...
    '<div>괴리율'+(d.premiumBasis==='regular'?'(정규장 기준)':d.premiumBasis==='close'?'(종가↔기준NAV)':'(시장가)')+' <b class="'+cls(d.premiumPct)+'">'+sign(d.premiumPct)+'%</b></div>'+
  ...
  document.getElementById('inav').innerHTML = fmt(d.inav,dp(d.inav))+'원 <span class="chg '+cls(d.inavChangePct)+'">'
    ...
    + (d.isOfficial ? ' <span class="badge official" style="font-size:12px;vertical-align:middle">KRX 공식</span>' : '');
...
if (!NODE) {
  globalThis.ENGINE = { computeINav, etfList, stats, cache, krSession, todayYmd, DEFAULT_CODE };
} else {
